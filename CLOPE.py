...
class Cluster:
    ...
    def __init__(self, history_count):
        ...
        # Площадь гистограммы
        self.area = 0.0
        # Высота гистограммы (в смысле H = S / W).
        # Данная величина нигде в явном виде не вычисляется.
        # Хранится в классе для полноты описания класса и не более того.
        self.height = 0.0
        # Ширина гистограммы (в смысле числа элементов)
        self.width = 0.0
        # Градиент (в смысле G = H / W). Данная величина
        # нигде в явном виде не вычисляется. Хранится в классе
    ...
    def add_transaction(self, transaction):
        ...
        for item in transaction:
            if not (item in self.histogram):
                ...
                self.histogram[item] += 1
        # Вчисляем суммарную площадь гистограммы
        # в смысле CLOPE (количество транзакций)
    ...
    def remove_transaction(self, transaction):
        ...
        Внутри класса не происходит слежение за тем, какие
        транзакции добавляются, какие удаляются, поэтому, если в
...
class CLOPE:
    ...
    def __init__(self,
        ...
        # Данный объект необходим для того,
        # чтобы не брать во внимание те объкты,
        # которые были отнесены к шумовым
        self.noise_clusters = {}
        ...
        # Словарь <ключ/значение> : №транзакции/№кластера
        self.transaction = {}
    ...
    def delta_transaction(self, transaction, cluster_number, r):
        ...
        будет искомым кластером (в который следует добавить
        транзакцию)

        Input parametres:
        transaction -- транзакция (список объектов)
        clusterNumber -- номер кластера, приращение,
    ...
    def get_goal_function(self, r):
        ...
        Input parametres:
        r -- вещественное число, обозначающие
    ...
    def move_transaction(self, transaction, id, repulsion=2, max_count_clusters=None):
        ...
        отталкивание кластеров в смысле CLOPE
        isSaveHistory -- флаг, выставляемый при необходимости
        записи истории количества транзакций
        Returned parameter:
        ...
        # Ищем кластер, на котором будет достигнуто
        # максимальное значение изменения целевой функции
    ...
    def init_clusters(self, data, repulsion=2,
        ...
        isPrint -- нужно ли печатать информацию о ходе
        выполнения (0 -- не нужно, если > 0 -- печатаем каждый isPrint раз)
        repulsion -- вещественное число, обозначающие
        отталкивание кластеров в смысле CLOPE
        isSaveHistory -- флаг, выставляемый при необходимости
        записи истории количества транзакций
        isNoiseReduction -- подавление шума (порог соответствует
        ...
        # Перемешивание по одному из признаков?
        np.random.seed(self.random_seed)
        np.random.shuffle(keys)

        for item in keys:
            self.move_transaction(data[item],
                                  item,
                                  repulsion,
                                  max_count_clusters)
            index += 1
        ...
        # Удаляем все шумовые кластеры (после инициализации
        # не бывает пустых классов, поэтому знак строго больше)
        if is_noise_reduction > 0:
            self.noise_reduction(is_noise_reduction)

        self.iteration = 1

    def next_step(self, data, repulsion=2,
                  is_noise_reduction=-1,
                  noise_median_threshold=0.75,
                  max_count_clusters=None):
        '''
        ...
        for id in keys:
            ...
            transaction = data[id]
            # Если транзакция относится к шумовому кластеру,
            # то не пытаемся её поменять
            if cluster_number in self.noise_clusters:
                ...
                # Рассматриваем транзакцию как вновь пришедшую и добавляем
                # в тот кластер, где значение целевой
                # функции доставит максимум
                eps += int(
                    self.move_transaction(transaction,
                                          id,
                                          repulsion,
                                          max_count_clusters)
                    !=
